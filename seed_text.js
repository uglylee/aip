...
function req(method, urlPath, body, headers = {}) {
  return new Promise((resolve, reject) => {
    ...
    const u = new URL('http://localhost:8005' + urlPath);
    const r = http.request({ hostname: u.hostname, port: u.port, path: urlPath, method, headers: h }, res => {
      let buf = '';
      res.on('data', c => buf += c);
...
const NAMES = ['Zhang','Wang','Li','Zhao','Chen','Liu','Yang','Huang','Zhou','Wu','Sun','Zhu','Ma','Hu','Lin','Angel','Luna','Max','Sofia','Leo','Mia','Aiden','Zoe','Lucas','Chloe','Ethan','Emma','Noah','Olivia','Jack','Ava','Logan','Henry','Sophia','Sebastian','Alexander','Charlotte','Daniel','Amelia','Matthew','Harper','Jackson','David','Carter','Emily','Jayden','Luke','Owen','Nathan','Grace'];

async function main() {
  // Test register
  const test = await req('POST', '/api/auth/register', { username: 'testprobe', handle: 'testprobe1', email: 'testprobe1@test.com', password: '123456' });
  console.log('Test register:', JSON.stringify(test).substring(0, 100));

  // Register 2 main users
  const r1 = await req('POST', '/api/auth/register', { username: 'zhaoli', handle: 'zhaoli', email: 'assd4750@163.com', password: 'Ab58576145' });
  const r2 = await req('POST', '/api/auth/register', { username: 'lihengju', handle: 'lihengju', email: 'lihengju@163.com', password: 'Ab58576145' });
  console.log('User 1:', r1.user ? r1.user.username : 'FAIL: ' + JSON.stringify(r1));
  console.log('User 2:', r2.user ? r2.user.username : 'FAIL: ' + JSON.stringify(r2));

  const allTokens = [r1.token, r2.token].filter(Boolean);

  // Register 98 more
  for (let i = 0; i < 98; i++) {
    const name = NAMES[i % NAMES.length];
    const handle = 'u' + Date.now() + i;
    try {
      const r = await req('POST', '/api/auth/register', {
        username: name + Math.floor(Math.random() * 9000 + 1000),
        handle, email: handle + '@test.com', password: '123456'
      });
      if (r.token) allTokens.push(r.token);
    } catch {}
  }
  console.log('Users:', allTokens.length);
  ...
  for (const t of allTokens) {
    ...
    for (let j = 0; j < count; j++) {
      ...
      try {
        await req('POST', '/api/posts', { content: text }, { Authorization: 'Bearer ' + t });
        total++;
      } catch {}
    }
  }
