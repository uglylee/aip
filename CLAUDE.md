...
- **应用更新**: dio 下载 APK + open_file + MethodChannel 原生安装
...
### 应用内更新
- 服务端 `/api/version` 返回版本号和 APK 下载地址
- 客户端 `update_service.dart` 对比版本，下载 APK 到本地
- Android 通过 MethodChannel + FileProvider + ACTION_VIEW 安装 APK
- 需要 `REQUEST_INSTALL_PACKAGES` 权限（Android 8.0+）
- `deploy.ps1` 自动递增版本号并更新服务端 `appVersion`
...
| `lib/services/update_service.dart` | 应用内版本检查 + 下载安装 |
| `lib/services/socket_service.dart` | WebSocket 连接管理 |
...
| `android/.../MainActivity.kt` | MethodChannel 处理 APK 安装 |
| `android/.../AndroidManifest.xml` | 权限声明 (含 REQUEST_INSTALL_PACKAGES) |
...
- Android 需要 `REQUEST_INSTALL_PACKAGES` 权限安装更新 APK
...
- `deploy.ps1` 的 taskkill 可能需要管理员权限（自动提权处理）
